...
export default function Navbar() {
  ...
  return (
    <>
    <header className="fixed top-0 left-0 right-0 z-50 w-full px-6 md:px-12 py-5 flex items-center justify-between pointer-events-none bg-black/40 [backdrop-filter:blur(25px)] [-webkit-backdrop-filter:blur(25px)] border-b border-white/10 md:bg-transparent md:[backdrop-filter:none] md:[-webkit-backdrop-filter:none] md:border-b-0">
      {/* Logo - Sol Taraf */}
      ...
      <div className="pointer-events-auto hidden md:flex flex-col items-center">
        <div className="flex items-center gap-1 bg-black/10 dark:bg-white/[0.08] border border-black/[0.12] dark:border-white/[0.15] [backdrop-filter:blur(20px)] [-webkit-backdrop-filter:blur(20px)] py-1 px-1 rounded-full shadow-[0_4px_24px_rgba(0,0,0,0.25)] relative max-w-fit">
          {navItems.map((item) => {
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    "text-black/70 hover:text-black dark:text-white/80 dark:hover:text-white [text-shadow:0_2px_4px_rgba(0,0,0,0.5)]",
                ...
                >
                  ...
                  {isActive && (
                    <motion.div
                      layoutId="lamp"
                      className="absolute inset-0 bg-white/15 dark:bg-white/10 rounded-full -z-10 ring-1 ring-inset ring-white/20"
                      initial={false}
                      transition={{
                        type: "spring",
                        stiffness: 300,
          ...
          <div className="border-l border-white/20 ml-2 pl-3 flex items-center h-8">
            <ThemeToggle />
          </div>
        </div>
        ...
        <AnimatePresence>
          {isHovered === servicesLabel && (
            <motion.div
              ...
              className="absolute top-full mt-3 w-[640px] p-6 bg-[rgba(10,10,15,0.92)] border border-white/[0.12] [backdrop-filter:blur(40px)_saturate(180%)] [-webkit-backdrop-filter:blur(40px)_saturate(180%)] rounded-[32px] shadow-[0_24px_64px_rgba(0,0,0,0.7),inset_0_1px_0_rgba(255,255,255,0.08)] z-50"
            >
              <div className="grid grid-cols-2 gap-4">
                {services.map((service) => (
                  ...
                  >
                    ...
                    <div>
                      <div className="text-sm font-bold text-white mb-1 [text-shadow:0_2px_4px_rgba(0,0,0,0.5)]">{service.title}</div>
      ...
      <button
        className="pointer-events-auto md:hidden flex items-center justify-center w-10 h-10 rounded-full bg-white/15 border border-white/20 text-white shadow-[0_2px_8px_rgba(0,0,0,0.3)]"
